...
if [ "$NFILE" -ne 1 ]; then
    sed -i -e "s#^FILENAME.*#FILENAME = \"snapdir_<snap>/snapshot_<snap>.<block>.hdf5\"#" "$ROCKSTAR_CONFIG_FILE"
fi
...
if [ $RUN_CLASS == 1 ]; then
    ...
    CLASS_JOB=`qsub -V -N $CLASS_RUN -q project -P hpc2501005 -k oe -l walltime=01:00:00 -l select=1:ncpus=16 -- $PREP_TRANSFER $CLASS_CONFIG_FILE $CLASS_TRANSFER_RAW $PYTHON_EXEC $CODE_HOME`
else
    echo "transfer function not requested"
    # no dummy needed since gadget has no dependency in this case
    # cd $HOME # this is user home
    # CLASS_JOB=`qsub -N dummy --q project -P hpc2501005 -k oe  -- $DUMMY_EXEC`
fi
...
if [ $RUN_SIM == 1 ]; then
    ...
    cd $GADGET_OUT_DIR
    if [ $RUN_CLASS == 1 ]; then
	GADGET_JOB=`qsub -V -N $GADGET_RUN -q project -P hpc2501005 -k oe -W depend=afterok:$CLASS_JOB -l walltime=$NHRS:$NMIN:00 -l select=$NNODE:ncpus=$NCPU:mpiprocs=$NCPU -- $GADGET_EXEC $GADGET_CONFIG_FILE $SIM_NPART $NCPU_TOT $NGENIC $RESTART $CODE_HOME`
    else
	# no dependency in this case
	GADGET_JOB=`qsub -V -N $GADGET_RUN -q project -P hpc2501005 -k oe -l walltime=$NHRS:$NMIN:00 -l select=$NNODE:ncpus=$NCPU:mpiprocs=$NCPU -- $GADGET_EXEC $GADGET_CONFIG_FILE $SIM_NPART $NCPU_TOT $NGENIC $RESTART $CODE_HOME`
    fi
else
    echo "simulation not requested"
    # no dummy needed since halos has no dependency in this case
    # cd $HOME # this is user home
    # GADGET_JOB=`qsub -N dummy -q project -P hpc2501005 -k oe -W depend=afterok:$CLASS_JOB  -- $DUMMY_EXEC`
fi
...
if [ $HALOS == 1 ]; then
    ...
    if [ $RUN_SIM == 1 ]; then
	ROCKSTAR_SERV_JOB=`qsub -V -N $ROCKSTAR_SERV -q project -P hpc2501005 -k oe -W depend=afterok:$GADGET_JOB -l walltime=05:00:00 -- $ROCKSTAR_EXEC $ROCKSTAR_CONFIG_FILE`
    else
	# no dependency on class
	ROCKSTAR_SERV_JOB=`qsub -V -N $ROCKSTAR_SERV -q project -P hpc2501005 -k oe -l walltime=05:00:00 -- $ROCKSTAR_EXEC $ROCKSTAR_CONFIG_FILE`
    fi
    # ADD INTERMEDIATE JOB HERE THAT SEARCHES FOR AUTO-ROCKSTAR.CFG FILE. 

    ROCKSTAR_PROC_JOB=`qsub -V -N $ROCKSTAR_PROC -q project -P hpc2501005 -k oe -W depend=after:$ROCKSTAR_SERV_JOB -l walltime=05:00:00 -l select=ncpus=$NWRITER -- $ROCKSTAR_EXEC $AUTO_ROCKSTAR_DIR/auto-rockstar.cfg`
else
    echo "halos not requested"
    # no dummy needed since trees won't be submitted and postproc has no dependency
    # ROCKSTAR_SERV_JOB=`qsub -N dummy -q project -P hpc2501005 -k oe -W depend=afterok:$GADGET_JOB  -- $DUMMY_EXEC`
fi
...
if [ $TREES == 1 ]; then
  ...
  GEN_CFG_JOB=`qsub -V -N $GEN_CFG -q project -P hpc2501005 -k oe -W depend=afterok:$ROCKSTAR_SERV_JOB -l walltime=00:10:00 -- $PERL_EXEC $CONSISTENT_TREES_GENCFG $ROCKSTAR_CONFIG_FILE`
  ...
  GEN_TREES_JOB=`qsub -V -N $GEN_TREES -q project -P hpc2501005 -k oe -W depend=afterok:$GEN_CFG_JOB -l walltime=05:00:00 -l select=ncpus=$NWRITER -- $PERL_EXEC $TREES_EXEC $CONSISTENT_TREES_CONFIG_FILE $CONSISTENT_TREES_DIR`
  GEN_CAT_JOB=`qsub -V -N $GEN_CAT -q project -P hpc2501005 -k oe -W depend=afterok:$GEN_TREES_JOB -l walltime=00:30:00 -- $PERL_EXEC $CATALOG_EXEC $CONSISTENT_TREES_CONFIG_FILE $CONSISTENT_TREES_DIR`

  CLEAN_TREE_EXEC=$SANDBOX_DIR/scripts/assist/clean\_trees.pl
  CLEAN_TREE_JOB=`qsub -V -N clean\_trees -q project -P hpc2501005 -k oe -W depend=afterok:$GEN_CAT_JOB -l walltime=00:10:00 -- $PERL_EXEC $CLEAN_TREE_EXEC $AUTO_ROCKSTAR_DIR $HOME`
else
  echo "trees not requested"
  # no need for dummy job since postproc doesn't need dependency
  # CLEAN_TREE_JOB=`qsub -N dummy -q project -P hpc2501005 -k oe -W depend=afterok:$ROCKSTAR_SERV_JOB  -- $DUMMY_EXEC`
fi
# #################
...
if [ $POSTPROCESS == 1 ]; then
    ...
    POSTPROC_JOB=`qsub -V -N $POSTPROC_RUN -J 1-$NJOBS_PP -q project -P hpc2501005 -k oe -l walltime=36:00:00 -l select=ncpus=$NCPU_PP -- $POSTPROC_EXEC $PYTHON_EXEC $SIM_FOLDER/$SIM_STUB $SIM_REAL $SIM_NPART $SNAP_START $N_OUT $PP_GRID $LBOX $NJOBS_PP $NCPU_PP` #  -l place=pack:exclhost
    # POSTPROC_JOB=`qsub -V -N $POSTPROC_RUN -J 1-$NJOBS_PP -q project -P hpc2501005 -k oe -W depend=afterok:$CLEAN_TREE_JOB -l walltime=36:00:00 -l select=ncpus=$NCPU_PP -- $POSTPROC_EXEC $PYTHON_EXEC $SIM_FOLDER/$SIM_STUB $SIM_REAL $SIM_NPART $SNAP_START $N_OUT $PP_GRID $LBOX $NJOBS_PP $NCPU_PP` #  -l place=pack:exclhost    
    qsub -N janitor -q project -P hpc2501005 -k oe -W depend=afterok:$POSTPROC_JOB -l walltime=00:10:00 -- $JANITOR $CLASS_OUT_DIR/$SIM_FOLDER $GADGET_OUT_DIR $AUTO_ROCKSTAR_DIR
else
    echo "post-processing not requested"
    # POSTPROC_JOB=`qsub -N dummy -q project -P hpc2501005 -k oe  -- $DUMMY_EXEC`
    # fix dependence on class/sims/halos/trees
    if [ $TREES == 1 ]; then
	qsub -N janitor -q project -P hpc2501005 -k oe -W depend=afterok:$CLEAN_TREE_JOB -l walltime=00:10:00 -- $JANITOR $CLASS_OUT_DIR/$SIM_FOLDER $GADGET_OUT_DIR $AUTO_ROCKSTAR_DIR
    elif [ $RUN_SIM == 1 ]; then
	qsub -N janitor -q project -P hpc2501005 -k oe -W depend=afterok:$GADGET_JOB -l walltime=00:10:00 -- $JANITOR $CLASS_OUT_DIR/$SIM_FOLDER $GADGET_OUT_DIR $AUTO_ROCKSTAR_DIR
    elif [ $RUN_CLASS == 1 ]; then
	qsub -N janitor -q project -P hpc2501005 -k oe -W depend=afterok:$CLASS_JOB -l walltime=00:10:00 -- $JANITOR $CLASS_OUT_DIR/$SIM_FOLDER $GADGET_OUT_DIR $AUTO_ROCKSTAR_DIR
