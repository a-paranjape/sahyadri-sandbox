...
if [ $POSTPROCESS == 1 ]; then
    ...
    ###########################
    POSTPROC_JOB=`qsub -V -N $POSTPROC_RUN -k oe -W depend=afterok:$CLEAN_TREE_JOB -l walltime=36:00:00 -l select=1:ncpus=1:mem=200gb -- $PYTHON_EXEC $POSTPROC_EXEC $SIM_FOLDER/$SIM_STUB $SNAP_START $N_OUT $SIM_REAL $PP_GRID $DOWN_SAMP $LBOX`
else
