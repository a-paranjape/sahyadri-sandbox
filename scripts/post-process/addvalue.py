...
class AddValue(HaloReader):
    ...
    ###############################################
    def add_value(self,write_vahc=False, only_tensor=False):
        """ Given matter (Fourier) density field, halo positions and halo properties, 
            ...
            if only_tensor is True, stores the tidal tensor in a .tens file
        """
        ...
        if only_tensor:
            ...
            for s in self.scale_strings:
                out_dtype.extend([(tstr+'11_'+s,'f'),(tstr+'22_'+s,'f'),(tstr+'33_'+s,'f'),(tstr+'12_'+s,'f'),(tstr+'13_'+s,'f'),(tstr+'23_'+s,'f')])
            va_props = np.zeros(self.Nhalo,dtype=out_dtype)
        else:
            va_props = np.zeros(self.Nhalo,dtype=self.vadtypelist)
        va_props['ID'] = self.halos['ID']

        if only_tensor is False:
            va_props['b1'] = self.calc_b1()

        out = self.calc_tensor(tensor='tidal', only_tensor=only_tensor)
        ...
        if only_tensor is False:
            ...
            out = self.calc_tensor(tensor='hessian', only_tensor=only_tensor)
            ...
            for s in self.scale_strings:
                va_props['tens11_'+s] = out['tens11_'+s]
                va_props['tens22_'+s] = out['tens22_'+s]
                va_props['tens33_'+s] = out['tens33_'+s]
                va_props['tens12_'+s] = out['tens12_'+s]
                va_props['tens13_'+s] = out['tens13_'+s]
                va_props['tens23_'+s] = out['tens23_'+s]
        ...
        if write_vahc:
            if (only_tensor is False):
                ...
                va_cat = self.sim_stem + '/r{0:d}/out_{1:d}.tens'.format(self.real,self.snap)
            ...
            if only_tensor is False:
                ...
                for s in self.scale_strings:
                    header_string += ' tens11_'+s+' tens22_'+s+' tens33_'+s+' tens12_'+s+' tens13_'+s+' tens23_'+s
    ...
    ###############################################
    def calc_b1(self):
        print("\n\n, in addvalue.py, we are calculating calc_b1\n")
    ...
    ###############################################
    def calc_tensor(self,tensor='tidal', only_tensor=False):
        """ if only_tensor=True, onlyt the tensor is calculated, and not its eigenvalues"""
        ...
        if (only_tensor is False):
            ...
            OUT = np.zeros([self.Nhalo, N_scales_this, 6], dtype=float) # 6 for the independent components of the tidal tensor
        ...
        for s in range(N_scales_this):
            ...
            for h in allowed_h:
                ...
                if (only_tensor is True):
                    OUT[h, s]=np.array([tens11_h, tens22_h, tens33_h, tens12_h, tens13_h, tens23_h])
                else:
        ...
        if (only_tensor is False):
            ...
            # shape (6,Nhalo,N_scales)
            tens_final = np.zeros((len(scale_strings_this),6,self.Nhalo),dtype=float)

            last_start = -3 if tensor == 'tidal' else -2
            for s in range(last_start,0):
                tens_final[s] = OUT[:,:,s].copy()
            ...
            if tensor == 'tidal':
                ...
                for s in range(self.N_scalestrings-3):
                    ...
                    for h in allowed_h:
                        for eig in range(6):
                            tens_final[s,eig,h] = np.interp(np.log10(RGeff[h]),np.log10(self.RGtab),OUT[eig,h,:self.N_RG])
                        if self.verbose:
                            self.status_bar(cnt,N_allowed)
                        cnt += 1
            ...
            for s in range(len(scale_strings_this)):
                out[tstr+'11_'+scale_strings_this[s]] = tens_final[s,0]
                out[tstr+'22_'+scale_strings_this[s]] = tens_final[s,1]
                out[tstr+'33_'+scale_strings_this[s]] = tens_final[s,2]
                out[tstr+'12_'+scale_strings_this[s]] = tens_final[s,3]
                out[tstr+'13_'+scale_strings_this[s]] = tens_final[s,4]
                out[tstr+'23_'+scale_strings_this[s]] = tens_final[s,5]
