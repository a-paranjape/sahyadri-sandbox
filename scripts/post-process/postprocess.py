...
def do_this_snap(snap):
    ...
    if calc_Pk:
        nbar_dm = (np.shape(pos))[1] / Lbox**3
        nbar_halos = halos.size / Lbox**3
        Pk_mm = ps.Pk_grid(FT_delta_dm,input_is_FTdensity=True, nbar = nbar_dm)

        delta_h = ps.density_field(hpos.T)
        FT_delta_h = ps.fourier_transform_density(delta_h)
        Pk_hh = ps.Pk_grid(FT_delta_h,input_is_FTdensity=True, nbar = nbar_halos)
