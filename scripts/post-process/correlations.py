...
class PowerSpectrum(Utilities,Paths,Constants):
    ...
    ###############################################
    def __init__(self,grid=256,Lbox=200.0,lgbin=True,nbin=30,KSEP=3,NFAC=10,logfile=None,verbose=True, dim=3):
        ...
        self.lib_ngp = cdll.LoadLibrary(self.python_path+'ngp_update.so')
    ...
    ################################################################
    def wtd_ngp_field(self,pos,weights=None,contrast=False):
        """ Compute (weighted) density field from particle locations using NGP.
            Assumes pos is numpy array with pos.shape = (dim,ndata)
            If weights is not None, assumes 1-d array of size ndata (weights used for each particle).
            Assumes position range along each dimension is (0,Lbox)
            Returns density (contrast) [shape (self.grid,self.grid,self.grid)]
        """
        ...
        if self.verbose:
            self.print_this('Computing (weighted) NGP field...',self.logfile)

        if weights is not None:
            if weights.shape != (ndata,):
                if self.verbose:
                    self.print_this('Incompatible shape for weights. Ignoring...',self.logfile)
                weights = np.ones(ndata,dtype=float)
            else:
                if self.verbose:
                    self.print_this('... using supplied weights',self.logfile)
        else:
            weights = np.ones(ndata,dtype=float)
            if self.verbose:
                self.print_this('... using unit weights',self.logfile)
        ...
        c_update_density = self.lib_ngp.update_density
        c_update_density.restype = None
